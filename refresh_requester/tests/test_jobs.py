...
def test_run_job_success(mocker, test_settings):
    ...
    result = run_job(test_date, limit=len(mocked_api_return_value))
    assert result == expected_jsonl_response


def test_run_job_request_exception_failure(mocker, test_settings):
    ...
    test_date = date(2025, 3, 1)
    result = run_job(test_date, limit=None)
    assert (
